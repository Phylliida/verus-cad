"""COLOR campaign: classify all 414,079 canonical equal-color profiles.

Tiers per profile (mirrors classify3d_all.py): torus sweep idx<=8 ->
box 3^3 -> box 4^3 (2M conflicts) -> torus <=32 -> STALL (deep probes
separately). Parallel, checkpointed (append jsonl, skip done on
restart — safe to relaunch after any death).

Run:  ./runpy.sh classify_color.py [workers=48]
"""
import json

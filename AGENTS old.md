...
- **Used for**:
  - Mathematical definitions and pure functions
  - Type invariants
  - Loop invariants (in exec/proof functions)
  - **Note**: `spec` functions **cannot** have `requires` or `ensures` clauses. Use `recommends` for lightweight preconditions instead.
- **Attributes**:
  - `spec(checked)` enables checking of `recommends` clauses

**Example**:
```rust
spec fn is_sorted(v: &Vec<u64>) -> bool
    recommends v.len() > 0  // optional, lightweight
{
...
Spec functions can use `recommends` to express optional preconditions. Unlike `requires`, which are mandatory, `recommends` are lightweight checks that help catch mistakes but are not enforced. By default, Verus does **not** check `recommends` on calls from other spec functions. Use the `spec(checked)` attribute to enable warnings for `recommends` violations.

- Calls from `proof` or `exec` functions **do** check `recommends`.
- If verification fails, Verus may automatically rerun with `recommends` checking to provide diagnostic notes.
- `recommends` are useful for documenting assumptions and catching incorrect usage during development.
...
### 7. Lessons from Polynomial and Field Extension Work
- Spec functions cannot contain imperative loops; use `Seq::new` and `summation::sum` for constructing sequences and sums.
- Returning a tuple `(a,b)` from a spec function works; `result.0` and `result.1` can be referenced in `ensures` clauses.
- Stubbing functions: Use `arbitrary()` and `assume(condition)` to temporarily admit values while preserving postconditions.
- Use `pub open spec fn` for spec functions that need a body and can be called across modules.
- Use `#[verifier::type_invariant]` to define type invariants.
- Use `old(expr)` to refer to the previous state of a mutable reference.

...
**nonlinear_arith discipline (learned the hard way in verus-physics2d):**
- **Never use `real`-valued NLA** — it diverges Z3 (>15 min hangs). Rational
  identities should be proved via `eqv_spec` unfolding to INTEGER
  cross-multiplication instead.
- **NLA ignores prior local asserts** (and even local `requires` facts are
  unreliable). Restate hypotheses INSIDE the assert as an implication:
  `assert(h1 && h2 ==> goal) by(nonlinear_arith);` (this is why
  verus-rational uses that style everywhere), or isolate the identity in a

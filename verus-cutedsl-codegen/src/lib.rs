...
#[allow(dead_code)]
fn emit_kernel_wgsl(k: &KernelDesc) -> String {
    ...
    for (i, arg) in func.sig.inputs.iter().enumerate() {
        if let FnArg::Typed(pat_type) = arg {
            if let Pat::Ident(pat_ident) = &*pat_type.pat {
                ...
                let is_output = name.starts_with("out_") || name.starts_with("output");
...
fn parse_workgroup_size(attr: TokenStream) -> [u32; 3] {
    ...
    if let Some(rest) = attr_str.strip_prefix("workgroup_size") {
        let rest = rest.trim();
...
///
/// Transforms a Rust function into a WGSL compute shader.
/// The original function is preserved for Verus verification,
/// and a companion `FOO_WGSL: &str` constant is generated.
///
/// # Example
///
/// ```ignore
/// #[kernel(workgroup_size = 256)]
/// fn vector_add(a: &[i32], b: &[i32], out_c: &mut [i32]) {
///     let idx = thread_idx();
///     out_c[idx] = a[idx] + b[idx];
/// }
/// // Generates: pub const VECTOR_ADD_WGSL: &str = "...";
/// ```
...
pub fn kernel(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let workgroup_size = parse_workgroup_size(attr);
    let params = extract_params(&func);

    // Generate WGSL body
    // For now: emit a simple element-wise kernel using global_invocation_id.x
    let body = format!(
        "  let tid = gid.x;\n  \
         // TODO: CuTe layout lowering will generate the body\n"
    );

    let wgsl_source = emit_wgsl_shader(&fn_name, &params, workgroup_size, &body);

    let const_name = syn::Ident::new(
        &format!("{}_WGSL", fn_name.to_uppercase()),
        func.sig.ident.span(),
    );
    ...
    let output = quote! {
        ...
        /// Auto-generated WGSL compute shader source.
        pub const #const_name: &str = #wgsl_source;
    };

...
module dilated(){ minkowski(){ raw(); sphere(r=r,$fn=8); } }
// closing = erode(dilated): fillets concave seams, keeps grooves > 2r open
difference(){
  translate(G) cube(BB, center=true);
  minkowski(){
    difference(){ translate(G) cube(BB, center=true); dilated(); }
    sphere(r=r,$fn=8);
  }
}
